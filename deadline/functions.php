...
function deadline_setup() {
	...
	register_nav_menus(
		array(
			'menu-1' => esc_html__( 'Primary', 'deadline' ),
		)
	);
	...
	add_theme_support(
		'custom-background',
		apply_filters(
			'deadline_custom_background_args',
			array(
...
function deadline_scripts() {
	wp_enqueue_style( 'deadline-style', get_stylesheet_uri(), array(), _S_VERSION );
	wp_style_add_data( 'deadline-style', 'rtl', 'replace' );

	wp_enqueue_script( 'deadline-navigation', get_template_directory_uri() . '/js/navigation.js', array(), _S_VERSION, true );
